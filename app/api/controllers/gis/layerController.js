...
// Live posts and pre-existing (archived) posts live in separate tables with
// different map-visibility rules, but share the same editing flow -- every
// check that used to hardcode "html_posts" must accept either.
  ...
  permissionController.hasPermission(req, res, "edit_layers", async () => {
    ...
    singleUpload(req, res, async function (err) {
      ...
      if (!decodedToken.roles.includes("admin_user") &&
        ...
        // Safe to interpolate payload.table here (not user-supplied SQL) --
        // this branch is only reachable when it's already been checked
        // against the HTML_POST_TABLES allowlist above.
